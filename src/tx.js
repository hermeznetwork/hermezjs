...
import { CONTRACT_ADDRESSES, GAS_LIMIT, GAS_MULTIPLIER } from './constants.js'
...
const deposit = async (
  ...
  const hermezContract = getContract(CONTRACT_ADDRESSES.Hermez, HermezABI, txSignerData, providerUrl)
...
const delayedWithdraw = async (
  ...
  const ethereumAddress = getEthereumAddress(hezEthereumAddress)
  const txSignerData = signerData || { type: SignerType.JSON_RPC, addressOrIndex: ethereumAddress }
  const delayedWithdrawalContract = getContract(CONTRACT_ADDRESSES.WithdrawalDelayer, WithdrawalDelayerABI, txSignerData, providerUrl)
