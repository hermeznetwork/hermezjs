...
test('Hermezjs sandbox', async () => {
  ...
  // TODO . sendL2Transaction is failing
  // send transaction to coordinator
  const l2TxExitResult = await hermez.Tx.sendL2Transaction(transaction,
	  hermezWallet.publicKeyCompressedHex)
...
/*
  ...
  var {transaction, encodedTransaction} = await hermez.TxUtils.generateL2Transaction( l2Tx,
	  hermezWallet.publicKeyCompressedHex,
	  srcAccount.token)
  hermezWallet.signTransaction(transaction, encodedTransaction)
  const XferResult = await hermez.Tx.sendL2Transaction(transaction, hermezWallet.publicKeyCompressedHex)
